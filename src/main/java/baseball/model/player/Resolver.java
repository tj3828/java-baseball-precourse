...
public class Resolver extends Player {
    ...
    private void validateInputRestart(String inputRestart) {
        if (!String.valueOf(BaseBallStatusType.RESTART.getCode()).equals(inputRestart)
            && !String.valueOf(BaseBallStatusType.EXIT.getCode()).equals(inputRestart)) {
            throw new IllegalArgumentException(
                "Please input value only " + BaseBallStatusType.RESTART.getCode() + " or "
                    + BaseBallStatusType.EXIT.getCode());
        }
    }
    ...
    private void validateInputAnswer(String inputAnswer) {
        if (!validateInputAnswerIfNumber(inputAnswer)) {
            throw new IllegalArgumentException(
                "Please input value only " + BaseBallConfig.BASEBALL_ANSWER_DIGITS_MIN_VALUE + " to "
                    + BaseBallConfig.BASEBALL_ANSWER_DIGITS_MAX_VALUE + " for each digit");
        }

        if (inputAnswer.length() != BaseBallConfig.BASEBALL_ANSWER_LENGTH) {
            throw new IllegalArgumentException(
                "Please input value only " + BaseBallConfig.BASEBALL_ANSWER_LENGTH + " digits");
        }
    ...
    private boolean validateInputAnswerIfNumber(String inputAnswer) {
        final String regex = "[" + BaseBallConfig.BASEBALL_ANSWER_DIGITS_MIN_VALUE + "-"
